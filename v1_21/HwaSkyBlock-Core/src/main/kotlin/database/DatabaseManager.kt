...
class DatabaseManager {
    ...
    fun insertSkyBlockStatic(skyblockId: String, name: String) {
        ...
        when (Config.get("database.type")) {
            "sqlite" -> {
                ...
                listOf(
                    "skyblock_door", "skyblock_chest", "skyblock_barrel", "skyblock_hopper",
                    "skyblock_furnace", "skyblock_blast_furnace", "skyblock_shulker_box",
                    "skyblock_trapdoor", "skyblock_button", "skyblock_anvil",
                    "skyblock_farm", "skyblock_beacon", "skyblock_minecart",
                    "skyblock_boat", "skyblock_pvp"
                ).forEach { key -> data[key] = "false" }

                data["skyblock_welcome_message"] = "Welcome to $skyblockId Skyblock!"
                data["skyblock_home"] = "0"
                data["skyblock_size"] = "0"
                data["skyblock_point"] = "0"
        ...
        Select_Skyblock_List[skyblockId] =
            hwaskyblock_skyblock(
                mapOf(
                    "skyblock_id" to skyblockId,
                    "skyblock_leader" to name,
                    "skyblock_point" to "0"
                )
            )
    }
    ...
    fun getSkyBlockDataStatic(skyblockId: String, type: String?): Any? {
        val dbType = Config.get("database.type")?.toString() ?: return null

        val typeToColumn = mapOf(
            "getSkyBlockName" to "skyblock_name",
            "getSkyBlockLeader" to "skyblock_leader",
            "isSkyBlockJoin" to "skyblock_join",
            "isSkyBlockBreak" to "skyblock_break",
            "isSkyBlockPlace" to "skyblock_place",
            "isSkyBlockDoor" to "skyblock_door",
            "isSkyBlockChest" to "skyblock_chest",
            "isSkyBlockBarrel" to "skyblock_barrel",
            "isSkyBlockHopper" to "skyblock_hopper",
            "isSkyBlockFurnace" to "skyblock_furnace",
            "isSkyBlockBlastFurnace" to "skyblock_blast_furnace",
            "isSkyBlockShulkerBox" to "skyblock_shulker_box",
            "isSkyBlockTrapdoor" to "skyblock_trapdoor",
            "isSkyBlockButton" to "skyblock_button",
            "isSkyBlockAnvil" to "skyblock_anvil",
            "isSkyBlockFarm" to "skyblock_farm",
            "isSkyBlockBeacon" to "skyblock_beacon",
            "isSkyBlockMinecart" to "skyblock_minecart",
            "isSkyBlockBoat" to "skyblock_boat",
            "isSkyBlockPvp" to "skyblock_pvp",
            "getSkyBlockWelcomeMessage" to "skyblock_welcome_message",
            "getSkyBlockHome" to "skyblock_home",
            "getSkyBlockSize" to "skyblock_size",
            "getSkyBlockPoint" to "skyblock_point",
            "isSkyBlockMonsterSpawn" to "skyblock_monster_spawn",
            "isSkyBlockAnimalSpawn" to "skyblock_animal_spawn",
            "isSkyBlockWeather" to "skyblock_weather",
            "isSkyBlockTime" to "skyblock_time",
            "isSkyBlockWaterPhysics" to "skyblock_water_physics",
            "isSkyBlockLavaPhysics" to "skyblock_lava_physics"
        )

        val column = typeToColumn[type] ?: return null

        val result = when (dbType.lowercase()) {
            "sqlite" -> {
                val dao = SkyblockDAO()
                dao.getSkyblock(skyblockId)
            }
    ...
    fun setSkyBlockDataStatic(skyblockId: String, value: Any?, type: String?) {
        ...
        val typeToColumn = mapOf(
            ...
            "setSkyBlockPoint" to "skyblock_point",
    ...
    fun loadAllSkyblocks() {
        val dbType = Config.getString("database.type") ?: return
        val resultMap = HashMap<String?, hwaskyblock_skyblock?>()

        when (dbType.lowercase()) {
            "sqlite" -> {
                val dao = SkyblockDAO()
                val list = dao.getAllSkyblocks()
                for (data in list) {
                    resultMap[data["skyblock_id"]?.toString()] =
                        hwaskyblock_skyblock(data.mapValues { it.value.toString() })
                }
            }
        ...
        Select_Skyblock_List = resultMap
        org.bukkit.Bukkit.getLogger().info("[DatabaseManager] ✅ Skyblock 데이터 ${Select_Skyblock_List.size}개 로드 완료")
    }
