...
data class AddonConfig(
    ...
    companion object {
        ...
        private fun parseFeeds(cfg: FileConfiguration): Map<String, FeedDefinition> {
            ...
            for (id in section.getKeys(false)) {
                ...
                map[id] = FeedDefinition(
                    ...
                    displayName = s.getString("display-name", "&a특수 사료")!!,
                    lore = s.getStringList("lore")
                )
            }
            return map
        ...
        private fun parseCattles(cfg: FileConfiguration): Map<String, CattleDefinition> {
            ...
            for (id in section.getKeys(false)) {
                ...
                map[id] = CattleDefinition(
                    ...
                    displayName = s.getString("display-name", "&f섬 가축")!!,
        ...
        private fun parseFeedRule(section: ConfigurationSection?): FeedRule {
            if (section == null) return FeedRule("특수사료", 5)
            return FeedRule(
                item = section.getString("item", "특수사료")!!,
        ...
        private fun parseBreedingRule(section: ConfigurationSection?): BreedingRule {
            if (section == null) return BreedingRule("번식토큰", 168, 600L)
            return BreedingRule(
                item = section.getString("item", "번식토큰")!!,
        ...
        private fun parseHologram(section: ConfigurationSection?): HologramRule {
            if (section == null) return HologramRule("&a🥕 {current}/{max}", "&e⏳ {hours}h")
            return HologramRule(
                feedFormat = section.getString("feed-format", "&a🥕 {current}/{max}")!!,
                breedFormat = section.getString("breed-format", "&e⏳ {hours}h")!!
            )
        }
    }
}
