...
class CattleStateStore {
    private val cache = ConcurrentHashMap<String, CattleState>()
    ...
    fun upsertAll(states: Collection<CattleState>) {
    ...
    fun deleteAll(entityUuids: Collection<String>) {
    ...
    private inline fun withConnection(block: (Connection, String) -> Unit) {
        ...
        // Core DB managers provide a shared long-lived connection.
        // Do not close it here, or other modules will fail with "database connection closed".
        block(conn, dbType)
    }
}
