...
class IslandMissionGUI(
    ...
    fun open(player: Player, islandId: String) {
        ...
        val inv = Bukkit.createInventory(null, size, color(MessageConfig.guiTitle))
            ...
            when (type) {
                ...
                "INFO" -> safeSet(inv, item.slot, infoItem(islandId))
                    ...
                    if (meta != null) {
                        ...
                        if (item.glow) meta.addEnchant(Enchantment.UNBREAKING, 1, true)
                        stack.itemMeta = meta
                    }
    ...
    private fun materialOrNull(name: String?): Material? {
        ...
        return Material.matchMaterial(key) ?: run {
            try {
    ...
    private fun applyDetailButtons(
        ...
        for (btn in config.guiDetailButtons) {
            ...
            if (meta != null) {
                ...
                if (btn.glow) meta.addEnchant(Enchantment.UNBREAKING, 1, true)
                stack.itemMeta = meta
            }
